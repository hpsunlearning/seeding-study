...
sc_dat_bb %>% group_by(birth_mode_sum, subjectid_unique) %>% summarise(N_samples = n()) %>% summarise(Mean_n_samples = mean(N_samples))
...
sc_dat_mm %>% group_by(subjectid_unique) %>% summarise(N_samples = n()) %>% summarise(Mean_n_samples = mean(N_samples))
```
