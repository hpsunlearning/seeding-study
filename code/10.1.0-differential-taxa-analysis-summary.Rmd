...
songbird_wk <- songbird %>% 
    select(body_site_days, featureid, 
           ...
           centered.C.birth_mode_ms..Treatment..Vag....T.CS.,
           centered.C.birth_mode_ms..Treatment..Vag....T.CSseed.,
    ...
    rename(P.VD = P.Vaginal., P.CSR = P.CS.seeded., P.CS = P.CS., 
           ...
           centered.Diff.CS = centered.C.birth_mode_ms..Treatment..Vag....T.CS.,
           centered.Diff.CSseed = centered.C.birth_mode_ms..Treatment..Vag....T.CSseed.) %>%
...
plot_ternary = function(bb, dd){
    ...
    dat_ternary$col = circlize::colorRamp2(seq(0, 1, 0.1), colors = colorspace::divergingx_hcl(11, palette = "Tropic"))(dat_ternary$seeding.effectiveness.scale)
...
image(x = seq(0,1,len=100), 
      ...
      cex.main = 1, 
      col = divergingx_hcl(100, palette = "Tropic"),
...
for (dd in c(2, 30, 120, 180)){
    ...
    ggplot(dat_ft, aes(x = ASV, y = rel_a, fill = birth_mode_ms)) +
        geom_boxplot() +
        coord_flip() +
        scale_x_discrete(breaks = dat$featureid, labels = dat$s) +
        geom_text(data = dat, aes(x = featureid, y = 1, label = centered.Diff.CS), inherit.aes = F)
