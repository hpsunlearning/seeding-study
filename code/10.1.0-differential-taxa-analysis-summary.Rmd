...
songbird_wk <- songbird %>% 
    select(body_site_days, featureid, 
           starts_with("P."), Taxon, C.birth_mode_ms..Treatment..Vag....T.CS., C.birth_mode_ms..Treatment..Vag....T.CSseed., feature.frequency, seeding.effectiveness) %>%
    rename(P.VD = P.Vaginal., P.CSR = P.CS.seeded., P.CS = P.CS., Diff.CS = C.birth_mode_ms..Treatment..Vag....T.CS., Diff.CSR = C.birth_mode_ms..Treatment..Vag....T.CSseed.) %>%
...
```{r}
bb = "Feces"
ggtern(songbird_wk %>% filter(body_site==bb), 
...
ggsave(paste0(fig_path, "Ternary_songbird_", bb, ".pdf"), width = 7, height = 9, useDingbats = F)
```
