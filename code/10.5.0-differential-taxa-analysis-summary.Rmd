...
plot_ternary = function(bb, dd, color_fun){
    TernaryPlot(point = "up",  alab = "P(Cesaren-seeded) <---", 
    ...
    AddToTernary(points, dat_ternary[, 1:3], pch = 21, cex = 0.7, col = dat_ternary$col, bg = dat_ternary$col)
}
...
for (bb in c("Feces", "Forearm", "Mouth")){
    ...
    dev.copy2pdf(file = paste0(fig_path, "songbird_ternary_plot_", bb, ".pdf"),
                 width = 3.5, height = 6, useDingbats = F, pointsize = 8)
    dev.off()
...
for (bb in c("Feces", "Forearm", "Mouth")){
    ...
    for (dd in c(2, 30, 180)){
        ...
        dat_sum <- dat %>% group_by(x) %>% summarise(med_seeding = median(seeding.effectiveness.scale), med_diff = median(centered.diff.CS), mean_seeding = round(mean(seeding.effectiveness.scale), 3))
        
        p_list[[as.character(dd)]] = ggplot(dat, aes(x = x, y = centered.diff.CS)) +
            coord_flip() +
            geom_bar(data = dat_sum, stat = "identity", aes(y = med_diff, fill = med_seeding), color = "grey", size = 0.1) +
    ...
    set_panel_size(g = patchworkGrob(p), file = paste0(fig_path, "songbird_diff_", bb, "_top", pct, "-g.pdf"), width = unit(1.3, "in"), height = unit(9/.pt*nrow(dat_sum), "mm"), useDingbats = F)
}
```
