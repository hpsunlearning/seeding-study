...
for (bb in c("Feces", "Forearm", "Mouth")){
    for (dd in c(2, 30, 120, 180)){
        ...
        x.order = dat %>% group_by(x) %>% summarise(MM = median(Diff.CS)) %>% arrange(desc(MM))
        
        p = dat %>% mutate(x = factor(x, levels = unique(x.order$x))) %>%
            arrange(x) %>%
            ggplot(aes(x = x, y = Diff.CS)) +
                coord_flip() +
            stat_summary(geom = "bar", fun = median, fill = "grey50") +
...
```{r}
load("tmp_all_phylo.Rdata")
songbird_flt <- songbird_wk2 %>% mutate(x = paste(p, c, o, f, g, sep = ";")) %>%
    group_by(body_site, days) %>%
    slice_max(seeding.effectiveness.scale, n = 20) %>%
    arrange(body_site, days, Taxon, desc(seeding.effectiveness.scale)) %>%
    select(body_site, days, featureid, p:s, seeding.effectiveness.scale, feature.frequency, P.VD, P.CSR, P.CS) %>%
    mutate(across(seeding.effectiveness.scale:P.CS, ~round(.x, 3)))
