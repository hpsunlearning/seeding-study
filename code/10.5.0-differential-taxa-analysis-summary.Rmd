...
for (bb in c("Feces", "Forearm", "Mouth")){
    ...
    for (dd in c(2, 30, 180)){
        ...
        dat_sum <- dat %>% group_by(p, c, o, f, g, x) %>% summarise(med_seeding = median(seeding.effectiveness.scale), med_diff = median(centered.diff.CS), mean_seeding = round(mean(seeding.effectiveness.scale), 3), max_seeding = max(seeding.effectiveness.scale)) %>% mutate(g_label = ifelse(g %in% c("-", "g_"), f, gsub("g_", "", g)))
