...
plot_ternary = function(bb, dd, color_fun){
    ...
    AddToTernary(points, dat_ternary[, 1:3], pch = 21, cex = 0.5, col = dat_ternary$col, bg = dat_ternary$col)
}

for (bb in c("Feces", "Forearm", "Mouth")){
    pdf(file = paste0(fig_path, "songbird_ternary_plot_", bb, ".pdf"), width = 8.5, height = 4, useDingbats = F, pointsize = 12)
...
for (bb in c("Feces", "Forearm", "Mouth")){
    ...
    for (dd in c(2, 30, 180)){
        ...
        dat_sum <- dat %>% group_by(x) %>% summarise(med_seeding = median(seeding.effectiveness.scale), med_diff = median(centered.diff.CS), mean_seeding = round(mean(seeding.effectiveness.scale), 3), max_seeding = max(seeding.effectiveness.scale))
        
        p_list[[as.character(dd)]] = ggplot(dat, aes(x = x, y = centered.diff.CS)) +
            coord_flip() +
            geom_bar(data = dat_sum, stat = "identity", aes(y = med_diff, fill = max_seeding), color = "grey", size = 0.1) +
            #geom_text(data = dat_sum[dat_sum$med_diff>0, ], aes(y = -0.1, label = mean_seeding), size = 6/.pt, hjust = 1) +
            #geom_text(data = dat_sum[dat_sum$med_diff<=0, ], aes(y = 0.1, label = mean_seeding), size = 6/.pt, hjust = 0) +
            scale_x_discrete(breaks = dat$x, labels = ifelse(dat$g %in% c("-", "g_"), paste(dat$f, dat$g, sep = ";"), dat$g)) +
    ...
    set_panel_size(g = patchworkGrob(p), file = paste0(fig_path, "songbird_diff_", bb, "_top", pct, "-g.pdf"), width = unit(1.3, "in"), height = unit(4,"in"), useDingbats = F)
