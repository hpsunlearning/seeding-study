...
for (i in seq_along(dat_files)){
    ...
    for (j in seq_along(dat_files[[i]])){
        ...
        dat_flt_[[grp]] = lapply(dat_files[[i]][[j]], function(x){x$W.taxa %>% as.data.frame()}) %>% data.table::rbindlist(., idcol = "Comp")
    }
    dat_flt[[nm]] = data.table::rbindlist(dat_flt_, idcol = "body_site_days")
}

write.table(dat_flt[["ASV"]], file = "../data/diff-analysis-new/ancom_all_results/ancom_w_country_pairwise_ASV_rsl.txt", sep = "\t", quote = F, row.names = F)
```
