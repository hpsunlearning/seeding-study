...
for (bb in c("Feces", "Forearm", "Mouth")){
    ...
    for (dd in c(2, 30, 180)){
        ...
        dat_sum <- dat %>% group_by(p, c, o, f, g, x) %>% summarise(med_seeding = median(seeding.effectiveness.scale), med_diff = median(centered.diff.CS), mean_seeding = round(mean(seeding.effectiveness.scale), 3), max_seeding = max(seeding.effectiveness.scale)) %>% mutate(g_label = ifelse(g %in% c("-", "g_"), paste(gsub("f_", "", f), g, sep = ";"), gsub("g_", "", g)))
        
        p_list[[as.character(dd)]] = ggplot(dat_sum, aes(x = x, y = med_diff)) +
            coord_flip() +
            geom_bar(stat = "identity", aes(fill = max_seeding), color = "grey", size = 0.1) +
            geom_text(aes(y = ifelse(med_diff>0, -0.1, 0.1), label = g_label, hjust = ifelse(med_diff>0, 1, 0)), size = 6/.pt) +
            scale_fill_continuous_sequential(palette = "Inferno", n_interp = 11, rev = F, begin = 0, end = 1, breaks = seq(0.1, 0.9, 0.2), limits = c(0, 1)) +
            theme_bw(base_size = 8) + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), plot.margin = margin(0, 0, 0, 0)) +
    ...
    set_panel_size(g = patchworkGrob(p), file = paste0(fig_path, "songbird_diff_", bb, "_top", pct, "-g.pdf"), width = unit(10/9, "in"), height = unit(3.5,"in"), useDingbats = F)
