...
dat_models = read_tsv("../data/diff-analysis-new/songbird-summary-output/TP-w-training-grid-search-all-allowed-models.tsv") %>% select(-X1) %>% filter(best=="Yes")
```
...
for (bb in c("Feces", "Forearm", "Mouth")){
    for (dd in c(2, 30, 180)){
        ...
        dat_phylo <-  all_phylo$ASV[[paste0("Baby-", bb, "-", dd, ".0")]]
        mt_phylo <- sample_data(dat_phylo) %>% as.matrix() %>% as.data.frame() %>% rownames_to_column(var = "sample_name")
        dat_ft <- dat_phylo %>% 
            ...
            filter(rel_a>0) %>%
            merge(., mt_phylo, by = "sample_name")
        dat_ft %>% group_by(birth_mode_ms, ASV) %>% summarise(Prevalanece = n())
        dat_ft %>% group_by(birth_mode_ms, ASV) %>% summarise(Prevalanece = n()) %>% pivot_wider(id_cols = ASV, names_from = birth_mode_ms, values_from = Prevalanece) %>% filter(is.na(Vag)) %>% nrow() %>% print()
