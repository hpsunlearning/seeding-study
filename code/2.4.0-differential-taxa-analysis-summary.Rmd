...
for (bb in c("Feces", "Forearm", "Mouth")){
    ...
    for (dd in c(2, 30, 180)){
        #dd=2
        dat <- songbird_wk2 %>% filter(body_site==bb, days ==dd, !featureid %in% decontam$Row.names, !featureid %in% decontam$Row.names) %>% mutate(x = paste(p, c, o, f, g, sep = ";"))
    ...
    p
    set_panel_size(g = patchworkGrob(p), file = paste0(fig_path, "songbird_diff_", bb, "_top", pct, "-g_CSref.pdf"), width = unit(2.1, "in"), height = unit(min(0.1*max_n, 4),"in"), useDingbats = F)
