...
ggplot(Convex_all, aes(x = birth_mode, y = convexhull_volume)) +
    ...
    theme_Pub(base_size = 10) + theme(aspect.ratio = 0.6) +
    labs(x = "", y = "Convex Hull (Volume)", title = "All")
set_panel_size(file = "../results/Figure-ConvexHull-R.pdf", width = unit(1.5, "in"), height = unit(0.9, "in"))
...
set_panel_size(file = "../results/Figure-ConvexHull-6mon-R.pdf", width = unit(1.5, "in"), height = unit(0.9, "in"))
```

```{r}
# By countries

Convex_6mon_2 <- Convex_6mon %>% mutate(country = substr(subjectid_unique, 1, 2), country = case_when(country =="BL" ~ "Bolivia", country %in% c("Ch", "CH") ~ "Chile", country  =="ES" ~ "Spain", T ~ "USA"))
for (cc in c("Chile", "Spain", "USA")){
    p = ggplot(Convex_6mon_2 %>% filter(country==cc), aes(x = birth_mode, y = convexhull_volume)) +
        geom_boxplot(outlier.shape = NA) +
        #geom_quasirandom(size = 1, width = 0.2, alpha = 0.5, aes(color = birth_mode), show.legend = F) +
        facet_wrap(.~body_site2, scales = "free_y") +
        scale_x_discrete(labels = c("Mom", "Vaginal", "Cesarean\n-seeded", "Cesarean")) +
