...
Feast_outs_sum_tbl2 <- Feast_outs_sum_tbl %>% filter(mom_body_site_corrected %in% c("Vagina", "Gauze")) %>% mutate(mom_body_site_corrected="Vaginal") %>%  group_by(BabySampleSite, mom_body_site_corrected) %>% summarise(across(body_site_ct:body_site_prop, sum)) %>% rbind(., Feast_outs_sum_tbl %>% filter(!mom_body_site_corrected %in% c("Vagina", "Gauze")) )

Feast_outs_sum_tbl_ <- Feast_outs_sum_tbl2 %>% separate(BabySampleSite, into = c("BabySampleID"), sep = "_", remove = F) %>% merge(., mf_st, by.x = "BabySampleID", by.y = "sample_name", all.x = T) %>% 
...
Feast_outs_sum_tbl_stats <- Feast_outs_sum_tbl_ %>% mutate(Group_day = ifelse(date_sampling_category_days_continuous<=2, 1, date_sampling_category_days_continuous)) %>% group_by(mom_body_site_corrected, body_site_type, Group_day) %>% dunn_test(data = ., body_site_prop ~ birth_mode_ms, p.adjust.method = "none") %>% mutate(p.adj.signif = NULL)

Feast_outs_sum_tbl_stats$p.adj = p.adjust(Feast_outs_sum_tbl_stats$p, method = "fdr")
Feast_outs_sum_tbl_stats$equal = ifelse(Feast_outs_sum_tbl_stats$p.adj<=0.1, F, T)

Feast_outs_sum_tbl_means <- Feast_outs_sum_tbl_ %>% mutate(Group_day = ifelse(date_sampling_category_days_continuous<=2, 1, date_sampling_category_days_continuous)) %>% group_by(mom_body_site_corrected, body_site_type, Group_day, birth_mode_ms) %>% summarise(mean = mean(body_site_prop)) %>% arrange(mom_body_site_corrected, body_site_type, Group_day, mean) %>% mutate(p.adj.l = letters[seq(n())])
...
Feast_outs_sum_tbl_ %>% mutate(Group_day = ifelse(date_sampling_category_days_continuous<=2, 1, date_sampling_category_days_continuous)) %>% group_by(mom_body_site_corrected, body_site_type, Group_day, birth_mode_ms) %>% summarise(mean = mean(body_site_prop)) %>% arrange(mom_body_site_corrected, body_site_type, Group_day, mean)
...
# names(ss_clr) <- c("Mouth", "Nose", "Forearm", "Right_Areola","Vaginal","Feces")
