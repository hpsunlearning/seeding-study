...
```
## Merge all things into a phyloseq object
```{r, eval=FALSE}
...
```{r}
BL_new = read.table(file = "../data/processed-data/Metadata_controls.txt", header = T, sep = "\t")
...
```{r}
phylo_bl <- prune_samples(BL_new$sample_name, phylo_all) 
ft_bl <- otu_table(phylo_bl) %>% as.matrix() %>% as.data.frame() %>% rownames_to_column(var = "OTU_ID") %>% filter(rowSums(.[,-1])!=0)
mt_ft_bl <- sample_data(phylo_bl) %>% as.matrix() %>% as.data.frame() %>% rownames_to_column(var = "sample_name") %>% mutate(seqcount = as.numeric(seqcount))

asv_ct <- ft_bl %>% summarise(across(mt_ft_bl$sample_name, ~sum(.x>0))) %>% t()
colnames(asv_ct) <- "ASV_ct"

asv_dat <- merge(asv_ct, mt_ft_bl, by.x = 0, by.y = 1, all.x  = T)
...
```
2. Similarity/distance comparison
```{r}
...
```
3. Taxa or ASV comparison
3.1 ASV prevalence
```{r}
# phylo_all
## creating a dictionary to map OTU id with an arbitrary index
dict_otu = data.frame(otu_id = rownames(tax_table(phylo_all))) %>% mutate(otu_ind = paste0("ASV", seq(n())))

mt_all = sample_data(phylo_all) %>% as.matrix() 
ft_all = otu_table(phylo_all) %>% as.matrix() %>% as.data.frame()
mt_ft_bl <- mt_all %>% as.data.frame() %>% filter(!is.na(Cat_control)) %>% rownames_to_column(var = "sample_name")

## asv prevalence in controls
ft_bl_l = ft_all[, mt_ft_bl$sample_name] %>% rownames_to_column("otu_id") %>% pivot_longer(names_to = "sample_name", values_to = "count", cols = -1) %>% filter(count>0) %>% group_by(sample_name) %>% mutate(rela = count/sum(count)) %>% left_join(., mt_ft_bl, by = "sample_name")
...
```{r}
mt_ft_bb <- mt_all %>% as.data.frame() %>% filter(mom_baby=="Baby") %>% rownames_to_column(var = "sample_name")
## asv prevalence in controls
ft_bb_l = ft_all[, mt_ft_bb$sample_name] %>% rownames_to_column("otu_id") %>% filter(rowSums(.[, -1])>0) %>% pivot_longer(names_to = "sample_name", values_to = "count", cols = -1) %>% filter(count>0) %>% group_by(sample_name) %>% mutate(rela = count/sum(count)) %>% left_join(., mt_ft_ss, by = "sample_name")
...
asv_prev_sum <- right_join(asv_prev_bl_sum, asv_prev_bb_sum %>% select(-otu_ind), by = "otu_id", suffix = c(".bl", ".bb")) %>% arrange(N.bl)
asv_prev_sum$otu_ind <- levels(asv_prev_sum$otu_ind, factor = asv_prev_sum$otu_ind)

ggplot(asv_prev_sum, aes(x = otu_ind, xend = otu_ind)) +
    geom_segment(aes(y = 0, yend = N.bb), color = "blue") +
    geom_segment(aes(y = 0, yend = N.bl*30), color = "red") +
    scale_y_continuous(name = "Samples", sec.axis = sec_axis(~./30, name = "Blanks")) +
    theme(axis.text.x = element_blank())

Hmisc::rcorr(asv_prev_sum$N.bl, asv_prev_sum$N.bb, type = "spearman")

ggplot(asv_prev_sum, aes(x = N.bl, y = N.bb)) +
    geom_point() +
    geom_smooth(method = "lm")

# filter appeared in more than 10% of each type, and had a med relative abundance in samples  >0.1%
asv_prev_sum_cand <- asv_prev_sum %>% filter(N.bl>16, N.bb>321, Med_rela.bb>0.001)

taxa_tbl <- tax_table(phylo_bl) %>% as.matrix %>% as.data.frame()

asv_prev_sum_cand_taxa <- asv_prev_sum_cand %>% merge(., taxa_tbl, by.x = "otu_id", by.y = 0, all.x = T)
```

There are 39817 ASVs identified in baby samples (3211 samples) and 1804 ASVs identified in blank samples (162 samples). 1134 ASVs appeared in both types of samples. The majority of those (1079 ASVs) appeared in less than 10 blank samples. 
