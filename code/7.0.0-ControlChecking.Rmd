...
3.1 ASV prevalence in control samples
```{r}
# phylo_all
...
3.2 ASV appeared in control samples prevlance in baby samples
```{r}
mt_ft_bb <- mt_all %>% as.data.frame() %>% filter(mom_baby=="Baby") %>% rownames_to_column(var = "sample_name")
## asv prevalence in controls
ft_bb_l = ft_all[, mt_ft_bb$sample_name] %>% rownames_to_column("otu_id") %>% filter(rowSums(.[, -1])>0) %>% pivot_longer(names_to = "sample_name", values_to = "count", cols = -1) %>% filter(count>0) %>% group_by(sample_name) %>% mutate(rela = count/sum(count)) %>% left_join(., mt_ft_bb, by = "sample_name")
...
ggsave("../results/controlvsamples_3.pdf", width = 11, useDingbats = F)
```
...
```{r}
# merge the baby asv table with control asv table together
asv_prev_sum_2 <- right_join(asv_prev_bl_sum, asv_prev_bb_os_sum %>% select(-otu_ind), by = "otu_id", suffix = c(".bl", ".bb")) %>% arrange(N.bl)
...
```{r}
# merge the baby asv table with control asv table together
asv_prev_sum_3 <- right_join(asv_prev_bl_sum, asv_prev_bb_f_sum %>% select(-otu_ind), by = "otu_id", suffix = c(".bl", ".bb")) %>% arrange(N.bl)
...
asv_prev_sum_2_cand <- asv_prev_sum_2 %>% filter(N.bl>16, N.bb>182, Med_rela.bb>0.001)

asv_prev_sum_3_cand <- asv_prev_sum_3 %>% filter(N.bl>16, N.bb>139, Med_rela.bb>0.001)
...
asv_prev_sum_2_cand %>% select(otu_ind.x, otu_id, N.bl, Avg_ct.bl, Med_ct.bl, Avg)
```
