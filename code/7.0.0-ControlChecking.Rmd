...
phylo_bl <- filter_taxa(phylo_bl, function(x) {sum(x)>0}, prune = T)

p <- plot_bar(phylo_bl, x = "Sample", fill = "Phylum")
#p + guides(fill = F) + facet_grid(.~Cat_control, scales = "free_x", space = "free_x") + scale_y_continuous(trans = "log10")

p_dat <- p$data %>% filter(Abundance>0)
```
```{r}
# ASV prevelance
asv_prev <- p_dat %>% group_by(Domain, Phylum, Class, Order, Family, Genus, Species, OTU) %>% summarise(N = length(unique(Sample)), Cat_bl = paste(unique(Cat_control), collapse = "-")) %>% mutate(Cat_bl = ifelse(nchar(Cat_bl)>8, "Both", Cat_bl)) %>% ungroup

ggplot(asv_prev %>% filter(N<=10), aes(x = N, y = 1)) +
    geom_bar(stat = "identity", aes(fill = Cat_bl))

ggplot(asv_prev %>% filter(N>10), aes(x = N, y = 1)) +
    geom_bar(stat = "identity", aes(fill = Cat_bl))


asv_prev_2 <-  p_dat %>% group_by(Domain, Phylum, Class, Order, Family, Genus, Species, OTU) %>% filter(length(unique(Sample))>20)

ggplot(asv_prev_2, aes(x = OTU, y = 1)) +
    geom_bar(stat = "identity", aes(fill = Cat_control)) +
    theme_bw() + theme(axis.text.x = element_blank())

otu_id <- data.frame(otu_seq = rownames(tax_table(phylo_bl))) %>% mutate(otu_no = paste0("ASV", seq(n())))

ggplot(asv_prev_sum, aes(x = N, y = Count)) +
    geom_line() +
    geom_point(size = 1) +
