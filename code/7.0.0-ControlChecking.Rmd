...
```{r}
# merge the baby asv table with control asv table together
asv_prev_sum_2 <- right_join(asv_prev_bl_sum, asv_prev_bb_os_sum %>% select(-c(Domain: otu_ind)), by = "otu_id", suffix = c(".bl", ".bb")) %>% mutate(Pct.N.bl = N.bl/162, Pct.N.bb = N.bb/1820) %>% arrange(N.bl)
...
ggplot(asv_prev_sum_2, aes(x = otu_ind)) +
    geom_line(aes(y = Pct.N.bb, group = 1, color = "Baby Samples")) +
    geom_line(aes(y = Pct.N.bl, group = 1, color = "Blank Samples")) +
    scale_y_continuous(n.breaks = 10) +
    scale_color_manual(breaks = c("Baby Samples", "Blank Samples"), values = c("#377eb8", "#e41a1c")) +
    theme_bw(base_size = 10) + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), aspect.ratio = 1, legend.position = c(0.01, 0.95), legend.justification = c(0, 1), plot.title = element_text(hjust = 0, size = 8)) +
...
```{r}
# merge the baby asv table with control asv table together
asv_prev_sum_3 <- right_join(asv_prev_bl_sum, asv_prev_bb_f_sum %>% select(-c(Domain: otu_ind)), by = "otu_id", suffix = c(".bl", ".bb")) %>% mutate(Pct.N.bl = N.bl/162, Pct.N.bb = N.bb/1391) %>% arrange(N.bl)
