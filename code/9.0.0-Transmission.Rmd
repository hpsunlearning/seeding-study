...
fun2 <- function(x, y){
    return(c(paste0(x, "T"), paste0(x, "F")))
}
tt <- "Vagina"
tt2 <- ""
for (ss_m in ss[-4]){
...
Grp_tbl <- data.frame(Index = tt2, Grp = tt)

mm_day0_asv_v = rownames(otu_mm_day0_lst$Vagina)
df_src <- data.frame(ASV = mm_day0_asv_v)
for (ss_m in ss[-4]){
    df_src[[ss_m]] = df_src$ASV %in% rownames(otu_mm_day0_lst[[ss_m]])
}
df_src <- df_src %>% mutate(across(Feces:Nose, ~ifelse(.x, "T", "F"))) %>% unite(mm.grp, Feces:Nose, sep = "", remove = F) %>% merge(., Grp_tbl, by.x = "mm.grp", by.y = "Index", all.x = T)
df_src$Grp[df_src$Grp=="Vagina"]="VaginaOnly"
...
for (bm in c("Vag", "CSseed", "CS")){
    tmp_tb1 <- asv_bb_tmp %>% filter(grepl(bm, sample)) %>% merge(df_src %>% select(ASV, Grp), by = "ASV", all.x = T) %>% mutate(date = str_extract(sample, "Day_\\d+"), bodysite = str_split(sample, pattern = "\\.", simplify = T)[, 1], xx = case_when(date=="Day_30" ~ 2, date=="Day_90" ~ 3, T ~ 4)) %>% arrange(date, sample, Grp, ASV) %>% group_by(date) %>% mutate(yy = seq(n()))

tmp_tb2 <- df_src %>% select(ASV, Grp) %>% mutate(sample = "Vagina", bodysite = "Vagina", date = "Day_0") %>% arrange(sample, Grp, ASV) %>% mutate(xx = 1, yy = seq(n()) + (max(tmp_tb1$yy)-nrow(df_src))/2, date = factor(date, levels = c("Day_0", "Day_30", "Day_90", "Day_240"))) %>% rbind(., tmp_tb1)

p_list[[bm]] <- ggplot(tmp_tb2, aes(x = date, y = yy)) +
    geom_line(aes(group = ASV, color = Grp), size = 0.2) +
    ...
    labs(x = "Days", y = "", fill = "Body Site", title = bm)
print(p_list[[bm]])
}

wrap_plots(p_list, ncol = 1) + plot_layout(guides = "collect") & theme(legend.position = "bottom", legend.box = "vertical")
ggsave("tmp_figs/asv_shared_mm_vagina.pdf", width = 11, useDingbats = F)
```
...
mm_day0_asv_v = rownames(otu_mm_day0_lst$Feces)
df_src <- data.frame(ASV = mm_day0_asv_v)
for (ss_m in ss[-1]){
    df_src[[ss_m]] = df_src$ASV %in% rownames(otu_mm_day0_lst[[ss_m]])
}
df_src <- df_src %>% mutate(across(Mouth:Nose, ~ifelse(.x, "T", "F"))) %>% unite(mm.grp, Mouth:Nose, sep = "", remove = F) %>% merge(., Grp_tbl, by.x = "mm.grp", by.y = "Index", all.x = T)
df_src$Grp[df_src$Grp=="Feces"]="FecesOnly"
...
tmp_tb2 <- df_src %>% select(ASV, Grp) %>% mutate(sample = "Feces", bodysite = "Feces", date = "Day_0") %>% arrange(sample, Grp, ASV) %>% mutate(xx = 1, yy = seq(n()) + (max(tmp_tb1$yy)-nrow(df_src))/2, date = factor(date, levels = c("Day_0", "Day_30", "Day_90", "Day_240"))) %>% rbind(., tmp_tb1)
...
wrap_plots(p_list, ncol = 1) + plot_layout(guides = "collect") & theme(legend.position = "bottom", legend.box = "vertical")
ggsave("tmp_figs/asv_shared_mm_feces.pdf", width = 11, useDingbats = F)
```
