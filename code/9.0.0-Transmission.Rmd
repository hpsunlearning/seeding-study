...
head(asv_bb_tmp)
asv_bb_tmp_sum <- asv_bb_tmp  %>% group_by(birthmode, date, bodysite, Grp2) %>% summarise(N = n()) %>% mutate(Pct = round(N/sum(N), 3), date = as.numeric(date)) %>% ungroup
...
ggsave("tmp_figs/asv_shared_mm_vagina_yr1.pdf", width = 9, useDingbats = F)
...
ggsave("tmp_figs/asv_shared_mm_feces_yr1.pdf", width = 9, useDingbats = F)
