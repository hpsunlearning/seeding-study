...
asv_bb_tmp_2_sum_stat <- asv_bb_tmp_2_sum %>% group_by(body_site_corrected, Grp2) %>% filter(!is.na(Grp2), length(unique(birth_mode_ms))>=2) %>% dunn_test(data = ., Rel_a ~ birth_mode_ms, p.adjust.method = "none") %>% mutate(p.adj.signif = NULL)
    
ggplot(asv_bb_tmp_2_sum %>% filter(!is.na(Grp2), date_sampling_category_days_continuous<=30), aes(x = date_sampling_category_days_continuous, y = Rel_a, color = birth_mode_ms)) +
    geom_point(size = 1, alpha = 0.5) +
...
```
```{r}
asv_bb_tmp_3 <- asv_bb_tmp_2 %>% filter(!is.na(Grp2)) %>%
    group_by(birth_mode_ms, body_site_corrected, date_sampling_category_days_continuous, Grp2, ASV) %>% summarise(Prev = length(unique(sample_name))) ##ASV prevalence

bb_ct <- mf_wk %>% filter(mom_baby=="Baby") %>% split(., list(.$birth_mode_ms, .$body_site_corrected, .$date_sampling_category_days_continuous)) %>% lapply(nrow) %>% do.call("rbind", .) %>% as.data.frame %>% rownames_to_column(var = "VV") %>% separate(VV, c("birthmode", "bodysite", "date"), sep = "\\.") %>% mutate(date = as.numeric(date))

asv_bb_tmp_3_1 <- asv_bb_tmp_3 %>% full_join(., bb_ct, by = c("birth_mode_ms"="birthmode", "body_site_corrected"="bodysite", "date_sampling_category_days_continuous"="date")) %>% mutate(Prevalence = Prev/V1) %>% arrange(birth_mode_ms, body_site_corrected, date_sampling_category_days_continuous, Grp2, Prevalence)

ggplot(asv_bb_tmp_3_1 %>% filter(Grp2=="Vagina.Feces.Others", date_sampling_category_days_continuous<=7), aes(x = ASV, y = Prevalence)) +
    geom_point() +
    facet_grid(body_site_corrected~birth_mode_ms, scales = "free_x") +
    theme(axis.text.x = element_blank())

asv_bb_tmp_3_2 <- asv_bb_tmp_3_1 %>% filter(Grp2=="Vagina.Feces.Others", date_sampling_category_days_continuous==0) %>% merge(., tax_table(phylo_wk), by.x = "ASV", by.y = 0, all.x = T) %>% arrange(desc(Prevalence))

ggplot(asv_bb_tmp_3_2 %>% filter(body_site_corrected=="Feces", Prevalence>=0.1), aes(x = paste(Order, Family, Genus, Species, sep = ";"), y = Prevalence, fill = birth_mode_ms)) +
    geom_bar(stat = "identity", position="dodge") +
    coord_flip()
```

```{r}
asv_mm_day0_filt_v_taxa <- merge(asv_mm_day0_filt_v, tax_table(phylo_wk) %>% as.matrix() %>% as.data.frame(), by.x = "ASV", by.y = 0, all.x = T)

asv_mm_day0_filt_v_taxa_tre <- split(asv_mm_day0_filt_v_taxa, asv_mm_day0_filt_v_taxa$birthmode) %>% lapply(., function(x){x %>% mutate(Grp2 = as.character(Grp2), Root = "Life") %>% unite(pathString, Root, Domain:Species, sep = "/", na.rm = T) %>% as.Node(.$pathString)})

print(asv_mm_day0_filt_v_taxa_tre[[1]], "Grp2", limit = NULL)
```
...
asv_mm_day0_filt_v <- asv_mm_day0_filt %>% filter(grepl("Feces", Grp)) %>% 
    mutate(Grp2 = case_when(Grp=="Feces" ~ "Feces.Only",
                            Grp %in% c("Feces.Nose", "Feces.Right_Areola") ~ "Feces.Others",
                            N_site <=2 ~ Grp,
                            grepl("Vagina.Feces.", Grp) ~ "Vagina.Feces.Others",
                            grepl("Feces.Forearm.", Grp) ~ "Feces.Forearm.Others",
                            T ~ "Feces.Others")) %>% 
...
ggplot(asv_bb_tmp_2_sum %>% filter(!is.na(Grp2)), aes(x = date_sampling_category_days_continuous, y = Rel_a, color = birth_mode_ms)) +
    geom_point(size = 1, alpha = 0.5) +
    stat_summary(geom="ribbon", aes(fill = birth_mode_ms), fun.data=mean_cl_boot, fun.args = list(conf.int=0.95), alpha = 0.3, color = NA) +
    stat_summary(geom="line", fun = mean, size = 1) +
    facet_grid(body_site_corrected ~ Grp2) +
